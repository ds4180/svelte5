...
<!-- ================================================================== -->
<div class="border-2 border-dashed border-base-300 rounded-xl p-5 bg-base-100 hover:border-base-content/40 transition-colors">

    <!-- 1. 업로드 완료된 파일 목록 -->
    {#if uploadedFiles.length > 0}
        <div class="mb-4">
            <p class="text-sm text-base-content/60 mb-2">첨부된 파일 ({uploadedFiles.length})</p>
            <div class="grid grid-cols-3 gap-3 pb-3 mb-3 border-b border-base-300 sm:grid-cols-4 md:grid-cols-6">
                {#each uploadedFiles as img, i}
                    <div class="relative group aspect-square rounded-lg overflow-hidden shadow-sm border border-base-300">
                        {#if img.thumbnail_filename}
                            ...
                            {#if onInsert}
                                <button
                                    type="button"
                                    class="absolute bottom-0 left-0 right-0 btn btn-xs btn-primary opacity-0 group-hover:opacity-100 transition-opacity rounded-none rounded-b-lg"
                        ...
                        {:else}
                            <!-- 이미지 아닌 문서 파일 미리보기 -->
                            <div class="w-full h-full flex flex-col items-center justify-center bg-base-200 p-1 text-center">
                                <span class="text-3xl">📄</span>
                                <span class="text-xs mt-1 break-all text-base-content/60">{img.original_name}</span>
                            </div>
                        {/if}
                        <!-- 삭제 버튼 -->
                        <button
    ...
    <div class="flex justify-center items-center gap-3">
        ...
        {#if files.length > 0}
            <button
                ...
                onclick={(e) => { e.preventDefault(); e.stopPropagation(); uploadFiles(); }}
                disabled={isLoading}>
                {#if isLoading}
                    <span class="loading loading-spinner loading-xs"></span> 업로드 중...
                {:else}
    ...
    {#if files.length > 0}
        <div class="mt-4">
            <p class="text-sm text-primary mb-2">업로드 대기 중... ({files.length})</p>
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
                {#each files as file, i}
                    <div class="relative group aspect-square rounded-lg overflow-hidden shadow-sm border-2 border-primary/30">
                        {#if file.type.startsWith("image/")}
                            <img src={previewUrls[i]} alt="미리보기" class="w-full h-full object-cover" />
                        {:else}
                            <div class="w-full h-full flex flex-col items-center justify-center bg-base-200 p-1 text-center">
                                <span class="text-3xl">📄</span>
                                <span class="text-xs mt-1 break-all text-base-content/60">{file.name}</span>
                            </div>
                        {/if}
                        <button
                            type="button"
